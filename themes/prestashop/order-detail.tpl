...
{if count($historyGls)}
	<h3>Suivre mon colis <small>(<a href="http://www.gls-group.eu/276-I-PORTAL-WEB/dLink.jsp?un=2502824801&pw=surdiscount&crf={$order->id}&no=3781001&lc=FR" target="_blank">Voir sur GLS</a>)</small></h3>
	<table class="detail_step_by_step std">
		<thead>
			<tr>
				<th class="first_item">Heures</th>
				<th class="item">Dépôt</th>
				<th class="last_item">État</th>
			</tr>
		</thead>
		<tbody>
			{foreach from=$historyGls item=item name="glsStates"}
				<tr class="{if $smarty.foreach.glsStates.first}first_item{elseif $smarty.foreach.glsStates.last}last_item{/if} {if $smarty.foreach.glsStates.index % 2}alternate_item{else}item{/if}">
					<td>{$item[0]}</td>
					<td>{$item[1]}</td>
					<td>{$item[3]}</td>
				</tr>
...
<p>
	...
	<a href="{$link->getPageLink('pdf-invoice.php')}?id_order={$order->id|intval}{if $is_guest}&secure_key={$order->secure_key}{/if}">{l s='Download your invoice as a .PDF file'}</a>
</p>
{/if}
{if $order->recyclable}
						...
						<td class="bold">
							<label for="cb_{$product.id_order_detail|intval}">
								{if $product.download_hash && $invoice}
									<a href="{$link->getPageLink('get-file.php')}?key={$product.filename|escape:'htmlall':'UTF-8'}-{$product.download_hash|escape:'htmlall':'UTF-8'}{if isset($is_guest) && $is_guest}&id_order={$order->id}&secure_key={$order->secure_key}{/if}" title="{l s='download this product'}">
