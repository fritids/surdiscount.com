...
<div id="last-products_block_center" class="block products_block">
	<h4><span class="left"></span><span class="middle">{l s='Nouveautées' mod='homelast'}</span><span class="right"></span></h4>
	{if isset($products) AND $products}
		<div class="block_content">
			...
			<ul>
			{foreach from=$products item=product name=homeLastProducts}
				<li class="ajax_block_product {if $smarty.foreach.homeLastProducts.first}first_item{elseif $smarty.foreach.homeLastProducts.last}last_item{else}item{/if} {if $smarty.foreach.homeLastProducts.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.homeLastProducts.iteration%$nbItemsPerLine == 1}clear{/if} {if $smarty.foreach.homeLastProducts.iteration > ($smarty.foreach.homeLastProducts.total - ($smarty.foreach.homeLastProducts.total % $nbItemsPerLine))}last_line{/if}">
	...
	{else}
		<p>{l s='No last products' mod='homelast'}</p>
	{/if}
</div>
<!-- /MODULE Home Last Products -->
