...
var ajaxCart = {
	...
	add : function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist){
		...
		/*	async: true,
			...
			success: function(jsonData)
			{
				...
				if ( $(callerElement).is('#crossselling_list a.exclusive, #accessorie_list a.exclusive') ) {
					elementToTransfert = $(callerElement).parents('li');
				}

				$('#overlay_cart').remove();
				$('body', document).append('<div id="overlay_cart"></div>');
				
				var offsetStart = elementToTransfert.offset();
				var widthStart = elementToTransfert.width();
				var heightStart = elementToTransfert.height();

				var offsetEnd = $('#cart_block').eq(0).offset();
				var widthEnd = $('#cart_block').eq(0).width();
				var heightEnd = $('#cart_block').eq(0).height();
	...
	updateCartEverywhere : function(jsonData) {
		...
		if ( jsonData.nbTotalProducts > 0 ) {
			...
			$('#cart_block_list .top, #cart_block_no_products').text(jsonData.nbTotalProducts+'x '+str_article);
		}
		else {
			$('#cart_block_list').html('<a href="'+baseDir+'order.php"><p id="cart_block_no_products">Aucun produit</p></a>\
			<a href="'+baseDir+'order.php">\
				<p id="cart-prices">\
			...
			<a href="'+baseDir+'order.php"></a>\
			<p id="cart-buttons">\
				<a title="Panier" class="button_small" href="'+baseDir+'order.php">Panier</a>\
				<a title="Commander" class="exclusive" id="button_order_cart" href="'+baseDir+'order.php?step=1">Commander</a>\
			</p>');
		}
	}
};
