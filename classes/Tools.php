...
class ToolsCore
{
		...
		if (!(isset($maintenance) AND (!in_array(Tools::getRemoteAddr(), explode(',', Configuration::get('PS_MAINTENANCE_IP'))))))
		{
			...
			elseif ($id_manufacturer = self::getValue('id_manufacturer'))
			{
				...
				if ($row)
				{
					...
					$row['meta_title'] = $row['meta_title'].' - '.'Hard Discount Papeterie, Fournitures de bureau, Fin de série, Destockage et divers... '.Configuration::get('PS_SHOP_NAME');
	...
	*/
	public static function getHomeMetaTags($id_lang, $page_name)
	{
		...
		$ret['meta_title'] = (isset($metas['title']) AND $metas['title']) ? $metas['title'].' - Hard Discount Papeterie, Fournitures de bureau, Fin de série, Destockage et divers... '.Configuration::get('PS_SHOP_NAME') : 'Hard Discount Papeterie, Fournitures de bureau, Fin de série, Destockage et divers... '.Configuration::get('PS_SHOP_NAME');
	...
	public static function completeMetaTags($metaTags, $defaultValue)
	{
		...
		if ($metaTags['meta_title'] == NULL)
			$metaTags['meta_title'] = $defaultValue.' - '.'Hard Discount Papeterie, Fournitures de bureau, Fin de série, Destockage et divers... '.Configuration::get('PS_SHOP_NAME');
