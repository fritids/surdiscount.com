...
//	Affiche d'un nuage de tag pour test !
	...
	$lang_mots_cles=_("Top %s des Mots-cles");
	echo "<h2>";
	printf($lang_mots_cles,$sps_config['display_mots_cles']);
	echo "</h2>\n";
	echo "<div>\n";
	echo "<a name=\"keyword\"></a>\n";
	
	
	$texte = _("Les mots cles sont les elements de la recherche qui ont permis a un visiteur d'arriver sur votre site. Ils sont classes par nombre de recherche depuis les differents moteurs disponible sur Internet. En cliquant sur l'icone a cote du mot, vous accederez a un lien permettant de faire une recherche sur plusieurs moteurs afin d'etudier votre positionnement pour un mot cle donne.Vous pourrez aussi voir l'evolution de ce mots cles sur les derniers jours.");
affiche_aide($texte,"themes/".$sps_config['default_theme']."/icones/help.png",$sps_config['aide']);

define("MIN_SIZE", 9);
define("MAX_SIZE", 30);

$req = @mysql_query("SELECT DISTINCT(mot_cle),COUNT(ip) as nbip FROM ".$sps_config['db_prefix']."statistiques WHERE $format_date_sql AND mot_cle != '' GROUP BY mot_cle HAVING COUNT(ip)>=".$sps_config['display_mots_cles_occurences']." ORDER BY nbip DESC LIMIT 0,".$sps_config['display_mots_cles'].";") or die(mysql_error());
...
if(count($tags))
	...
	foreach ($tags as $tag) {
		...
		$tags_extended[] = $tag;
		$last_id = @mysql_result(mysql_query("SELECT id FROM ".$sps_config['db_prefix']."statistiques WHERE $format_date_sql AND mot_cle='".addslashes($tag['mot_cle'])."' ORDER BY id DESC LIMIT 0,1;"),0,"id");
