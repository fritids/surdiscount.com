...
class AdminProducts extends AdminTab
{
		...
		if (!sizeof($this->_errors))
		{
			...
			if (isset($id) AND !empty($id))
			{
				...
				if (Validate::isLoadedObject($object))
				{
					...
					if ( !empty( $_FILES['new_etiquette']['name'] ) ) {
						$path_etiquette = _PS_IMG_DIR_.'et'.DS;
						$file_etiquette = time().'-'.$_FILES['new_etiquette']['name'];

						$object->etiquette = $file_etiquette;

						move_uploaded_file( $_FILES['new_etiquette']['tmp_name'], $path_etiquette.$file_etiquette );
					}
					elseif ( !empty( $_POST['etiquette'] ) ) {
						$object->etiquette = $_POST['etiquette'];
					}
					else {
						$object->etiquette = '';
					}
	...
	function displayFormInformations($obj, $currency)
	{
		...
		<div class="tab-page" id="step1">
			...
			<script src="../js/admin_modele.js" type="text/javascript"></script>
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							<select name="condition" id="condition">
								...
								<option value="generique" '.($obj->condition == 'generique' ? 'selected="selected"' : '').'>'.$this->l('Generique').'</option>
							</select>
						</td>
					</tr>
					<tr>
		...
		echo '
					...
					<tr>
						<td colspan="2"><h3>'.$this->l('Espace cartouches').'</h3></td>
					</tr>
					<tr>
						<td style="vertical-align:top;text-align:right;padding-right:10px;font-weight:bold;">'.$this->l('Tags:').'</td>
						<td style="padding-bottom:5px;" class="translatable">';
							if ($obj->id)
								$obj->tags = Tag::getProductTags((int)$obj->id);

							foreach ($this->_languages as $language) {
								echo '<div data-id_lang="'.$language['id_lang'].'" class="tags lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
									<input size="45" type="text" name="add_tag" id="add_tag" class="add_tag" autocomplete="off" />
									<input class="all_tags" type="hidden" id="tags_'.$language['id_lang'].'" name="tags_'.$language['id_lang'].'"
									...
									<ul class="list_current_tags">';
										if ( count( $obj->tags[$language['id_lang']] ) ) {
											foreach ( $obj->tags[$language['id_lang']] as $item ) {
												echo '<li><a>x</a><span>'.$item.'<input type="hidden" class="tag" value="'.$item.'" /></span></li>';
											}
										}
					...
					<tr>
						<td style="vertical-align:top;text-align:right;padding-right:10px;font-weight:bold;">'.$this->l('Tinkco: ').'</td>
						<td style="padding-bottom:5px;" class="translatable">';
							foreach ($this->_languages as $language) {
								echo '<div data-id_lang="'.$language['id_lang'].'" class="tinkco lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
									<input type="text" name="tinkco" class="tinkco" autocomplete="off" /><div class="ok">'.$this->l('Rechercher').'</div>
								</div>';
							}
						echo '</td>
					</tr>
				...
				?>
					...
					<?php if ( $obj->id ) : ?>
						<tr><td colspan="2"><h3><?php echo $this->l('Etiquettes'); ?></h3></td></tr>
						<tr><td><?php echo $this->l('Etiquettes:'); ?></td><td><input type="file" name="new_etiquette" /></td></tr>
						<tr>
							<td colspan="2">
								<?php $etiquettes = glob( _PS_IMG_DIR_.'et'.DS.'*.*' ); ?>
								<?php foreach ( $etiquettes as $item ) : ?>
									<?php $item = str_replace( _PS_ROOT_DIR_, '', $item ) ?>
	
									<div class="etiquette<?php if ( preg_match( '#'.$obj->etiquette.'#', $item ) && !empty( $obj->etiquette ) ) echo ' selected'; ?>">
										<img width="10%" src="<?php echo $item; ?>" data-src="<?php echo basename( $item ); ?>" />
										<span class="delete">X</span>
									</div>
								<?php endforeach; ?>
								
								<input type="hidden" name="etiquette" id="etiquette" value="<?php echo $obj->etiquette; ?>" />
	
								<div class="clear"></div>
	
								<script type="text/javascript">
									$('.etiquette img').live('click', function () {
										var $this = $(this);
	
										$('.etiquette img').parent().removeClass('selected');
										$this.parent().addClass('selected');
	
										if ( $this.data('src') == $('#etiquette').val() ) {
											$('input#etiquette').val( '' );
											$('.etiquette img').parent().removeClass('selected');
										}
										else {
											$('input#etiquette').val( $this.data('src') );
										}
										
									});
									
									$('.etiquette .delete').live('click', function () {
										var $this = $(this);
	
										if ( !confirm('Souhaitez vous supprimer l\'image ?') ) {
											...
											$.ajax({
												url: 'ajax.php',
												data: { 'ajaxDeleteEtiquette': 1, 'img': $this.parent('div').find('img').attr('src') },
												success: function() {
