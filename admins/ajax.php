...
if (isset($_GET['ajaxDeleteEtiquette']))
{
	unlink( _PS_ROOT_DIR_.$_GET['img'] );
}
...
if ( Tools::getValue( 'ajaxModele' ) ) {
	$Tag = new Tag;
	
	$id_manufacturer = Tools::getValue( 'id_manufacturer' );
	
	$tags = explode( ', ', Tools::getValue( 'q' ) );
	
	$q = trim( end( $tags ) );
	
	$datas = array();
	if ( !empty( $q ) ) {
		$data = $Tag->findTag( (int)($cookie->id_lang), $q, $id_manufacturer );
